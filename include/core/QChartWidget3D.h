...
private:
    ...
    // QChartWorldBox m_worldBounds;                       —— 旧名/旧类型，随 3D 系列阶段删除
    ...
    // series3D 数据包围盒/反算（recomputeDataBounds3D/computeSeriesDataBounds/...）—— 待 3D 系列阶段恢复
    // （批次 B2 补全，按旧 HEAD 头逐项核对）
    // layers3D() 列表访问器 / m_layers3D —— 现为单 layer3D 托管（layer3D() 替代；多 3D 层待 3D 容器扩展阶段）
    // dataBounds3DMin()/dataBounds3DMax() / m_dataBounds3D* —— 域盒访问器经 layer3D 数据盒/axes3D() 获取；
    //   如需保留同名便捷可在 Widget 数据链阶段补
    ...
    // mouse*/wheel/leaveEvent/resizeEvent 覆写 + m_anchorBox —— 待交互阶段恢复（基类事件已统一 final 分发）
};
