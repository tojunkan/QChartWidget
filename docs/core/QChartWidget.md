...
| `QPointF` | `pixelToCartesian` | Pixel → View Cartesian（逆映射转发）。 | `const QPointF& pixel` | public | — | 交互（pan/滚轮中心）/命中 | `QChartCamera2D` |
| `void` | `setViewRect` | 绝对设置 viewRect；自动反算 dataBounds + invalidate + 发 viewChanged。 | `const QRectF& r` | public | — | `QViewRectAnimation`/`setDataRangeDim0/1`（经相机）/用户 | `QChartCamera2D` <br> `QChartProjection` |
...
| `void` | `invalidateBackground` | 背景缓存失效 → update（触发 paintEvent）。 | 无 | public | — | 轴/网格变化相关槽、用户 | `QChartRenderer` |
| `void` | `invalidateForeground` | 前景缓存失效 → update。 | 无 | public | — | 系列/图例变化相关槽、用户 | `QChartRenderer` |
| `void` | `invalidateLayout` | 布局脏标记置位 → update。 | 无 | public | — | addAxis/removeAxis 等 | — |
| `QChartScene` | `buildScreenScene` | 组装屏显场景快照（§8.2 钩子，3D 子类重写注入 3D 段）。 | 无 | protected virtual | — | `paintEvent` | `QChartScene` <br> `QChartRenderer` |
...
| `void` | `layoutAxes` | 按 margins 与 sizeHint 重算 plotArea（不动 viewRect）。 | 无 | protected virtual | — | `resizeEvent`/`paintEvent`/`setMargins` | — |
| `void` | `fitViewRectToPlotArea` | 触发相机 fit 几何 + 反算 dataBounds（避免 Polar 往返漂移）。 | `FitStrategy strategy` | protected | — | `setViewRect`（经布局）/`setProjection`/`setDataRangeDim0/1` | `QChartCamera2D` <br> `QChartProjection` |
...
| `bool` | `dimensionInteractive` | 该维度是否允许交互（任一绑定轴 isInteractive()==false → 禁止，如分类轴）。 | `int dim` | protected | `true`/`false` | `mouseMoveEvent`/`wheelEvent` | `QChartAxis` |
...
| `void` | `rebuildLegendItems` | 重建图例条目（汇总所有 layer、跳过空 name、按 add 顺序）。 | 无 | private | — | `addLayer`/`removeLayer` 的系列增减槽 | `QChartSeries` <br> `QChartLegend` |
| `QRectF` | `plotAreaForSize` | 给定尺寸下重算 plotArea（复用 layoutAxes 逻辑，供 WholeWidget 导出）。 | `const QSize& size` | private | — | `layoutAxes`/`buildExportScene` | — |
...
| `viewChanged` | 视图状态变化广播：`setViewRect`/`panViewCartesian`/`zoomViewCartesian`/`setDataRangeDim0/1` 均 `emit viewChanged()`。 | — | NULL（内部未连线；测试/demo 按需连接） | `QChartCamera2D` <br> `QChartProjection` |
