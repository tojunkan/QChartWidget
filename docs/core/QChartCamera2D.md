...
| `ViewRectFitMode` | `m_fitMode` | fit 模式（`setFitMode` 修改）。 | `Stretch` <br> `Expand` <br> `Crop` <br> `Preserve` | `ViewRectFitMode::Preserve` | — |
| `qreal` | `m_scale` | 在Expand或Crop后进行缩放的系数 | `qreal` | `1.0` | — |

Notes:
- `ViewRectFitMode` 与 `FitStrategy`（KeepTopLeft/KeepBottomRight/KeepTop/KeepBottom/KeepRight/KeepLeft/KeepTopRight/KeepBottomLeft/KeepCenter）均为本头文件级枚举（非类内）。
...
| `qreal` | `scale` | Fixed 长宽比访问器（内联）。 | 无 | public | — | 测试 | — |
| `void` | `setScale` | 设置 Fixed 长宽比（内联）。 | `qreal ratio` | public | — | 用户 | — |
