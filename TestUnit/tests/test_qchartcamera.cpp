// test_qchartcamera.cpp —— QChartCamera 单元测试
...
void TestQChartCamera::cartesianToPixel_roundtrip() {
        ...
        for (const QRectF& pa : plotAreas) {
            for (int i = 0; i <= 10; ++i) {
                for (int j = 0; j <= 10; ++j) {
                    ...
                    QPointF px = QChartCamera::cartesianToPixel(vr, pa, cx, cy);
                    QPointF back = QChartCamera::pixelToCartesian(vr, pa, px);
...
void TestQChartCamera::pixelToCartesian_roundtrip() {
        ...
        for (const QRectF& pa : plotAreas) {
            for (int i = 0; i <= 8; ++i) {
                for (int j = 0; j <= 8; ++j) {
                    ...
                    QPointF c = QChartCamera::pixelToCartesian(vr, pa, px);
                    QPointF back = QChartCamera::cartesianToPixel(vr, pa, c.x(), c.y());
...
// ===== pan 只平移、不改尺寸 =====
void TestQChartCamera::pan_preservesSize() {
    QChartCamera cam;
...
void TestQChartCamera::zoom_keepsCenterFixedPoint() {
    QChartCamera cam;
    cam.setViewRect(QRectF(0, 0, 10, 20));
...
void TestQChartCamera::fitMode_Stretch_noChange() {
    QChartCamera cam;
    ...
    bool changed = cam.fitViewRectToPlotArea(QRectF(0, 0, 400, 200),
                                             QChartCamera::FitStrategy::KeepCenter);
    QVERIFY(!changed);
    QCOMPARE(cam.viewRect(), before);
}

// ===== Fit：扩张较小维度匹配 plotArea 长宽比，数据完整 =====
void TestQChartCamera::fitMode_Fit_expands() {
    QChartCamera cam;
...
void TestQChartCamera::fitMode_Crop_shrinks() {
    QChartCamera cam;
...
void TestQChartCamera::fitMode_Fixed_aspectRatio() {
    QChartCamera cam;
...
void TestQChartCamera::centerZoom_propertyConsistency() {
    QChartCamera cam;
